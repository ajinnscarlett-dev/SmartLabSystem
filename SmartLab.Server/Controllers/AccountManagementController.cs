...
public class AccountManagementController : ControllerBase
{
    ...
    public AccountManagementController(AppDbContext context) => _context = context;
    ...
    {
        if ((request.NewPassword ?? string.Empty).Length < 6) return BadRequest(new { message = "Password must be at least 6 characters." });
        var user = await _context.Users.FirstOrDefaultAsync(u => u.UserId == id);
        if (user == null) return NotFound(new { message = "User not found." });
    ...
    {
        _context.ActivityLogs.Add(new ActivityLog { UserId = userId, PCId = null, Action = action, Details = details, CreatedAt = DateTime.Now });
        await _context.SaveChangesAsync();
    }
    ...
    private static bool IsDisabled(User u) => u.Username.StartsWith(DisabledPrefix, StringComparison.Ordinal);
    ...
    private static string PackDisabledUsername(string username) => DisabledPrefix + Convert.ToBase64String(Encoding.UTF8.GetBytes(username)) + "|" + Guid.NewGuid().ToString("N");

    public sealed class CreateAccountRequest { public string Username { get; set; } = string.Empty; public string Password { get; set; } = string.Empty; public string Role { get; set; } = "Student"; }
    public sealed class RenameRequest { public string Username { get; set; } = string.Empty; }
    public sealed class ChangeRoleRequest { public string Role { get; set; } = string.Empty; }
    public sealed class PasswordResetRequest { public string NewPassword { get; set; } = string.Empty; }
}
