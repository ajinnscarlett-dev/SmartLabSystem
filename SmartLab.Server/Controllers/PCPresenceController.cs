...
namespace SmartLab.Server.Controllers
{
    ...
    public class PCPresenceController : ControllerBase
    {
            ...
            [FromBody] MachinePresenceRequest request)
        {
            ...
            if (string.IsNullOrWhiteSpace(pcNumber) || string.IsNullOrWhiteSpace(macAddress))
            ...
            var pc = await _context.PCs.FirstOrDefaultAsync(p => p.PCNumber == pcNumber);
            if (pc == null)
                return NotFound(new { message = "This workstation is not registered." });

            string normalizedIncomingMac = NormalizeMac(macAddress);
            string? normalizedStoredMac = string.IsNullOrWhiteSpace(pc.MACAddress) ? null : NormalizeMac(pc.MACAddress);
            ...
            bool macBelongsElsewhere = await _context.PCs.AnyAsync(p =>
                p.PCId != pc.PCId && p.MACAddress != null && p.MACAddress.ToLower() == normalizedIncomingMac.ToLower());
        ...
        private static string NormalizeMac(string value) => value.Trim().Replace(":", string.Empty).Replace("-", string.Empty).Replace(".", string.Empty).ToUpperInvariant();

        private static bool IsValidMac(string value)
        {
            string normalized = NormalizeMac(value);
            return normalized.Length == 12 && normalized.All(c => (c >= '0' && c <= '9') || (c >= 'A' && c <= 'F'));
        }
    }
}
